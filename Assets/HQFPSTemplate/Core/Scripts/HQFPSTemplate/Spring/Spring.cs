...
namespace HQFPSTemplate
{
	...
	public class Spring
	{
		...
		private readonly float m_LerpSpeed;
		private readonly Vector3[] m_DistributedForce = new Vector3[100];
		...
		{
			if (m_LerpSpeed > 0f)
				m_LerpedPosition = Vector3.Lerp(m_LerpedPosition, m_Position, Time.deltaTime * m_LerpSpeed);
			else
		...
		public void AddDistributedForce(SpringForce force) => AddDistributedForce(force.Force, force.Distribution);
		...
		private void UpdatePosition() => m_Position = Vector3Utils.GetNaNSafeVector3((m_Position + m_Velocity));
