...
namespace HQFPSTemplate.Equipment
{
	...
	public partial class ProjectileWeaponVFX : PlayerComponent, IEquipmentComponent, IObjectReferenceFiller
	{
		...
		public void Initialize(EquipmentItem equipmentItem)
		{
			...
			int minPoolSize = m_Weapon.MagazineSize;
			int maxPoolSize = minPoolSize * 2;

			if (m_VFXInfo.ParticleEffects.MuzzleFlashPrefab != null)
				PoolingManager.Instance.CreatePool(m_VFXInfo.ParticleEffects.MuzzleFlashPrefab, minPoolSize, maxPoolSize, true, m_VFXInfo.ParticleEffects.MuzzleFlashPrefab.GetInstanceID().ToString(), 2f);
			...
			if (m_VFXInfo.MagazineEjection.MagazinePrefab != null)
				PoolingManager.Instance.CreatePool(m_VFXInfo.MagazineEjection.MagazinePrefab, 1, 2, true, m_VFXInfo.MagazineEjection.MagazinePrefab.GetInstanceID().ToString(), 15f);
