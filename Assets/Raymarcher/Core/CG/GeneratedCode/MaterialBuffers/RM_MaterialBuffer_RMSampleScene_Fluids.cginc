...
struct RMStandardPBRData
{
    RMStandardLitData litData;
    half reflectionIntensity;
    half reflectionJitter;
    half refractionOpacity;
    half refractionIntensity;
    half refractionDensity;
    half refractionInverse;
};

StructuredBuffer<RMStandardPBRData> RMStandardPBRDataInstance;

// --------------------- Registered Texture Containers
Texture2DArray RMStandardPBRDataTextures;
SamplerState sampler_RMStandardPBRDataTextures;
...
half4 CalculatePBRModelWrapper(in RMStandardPBRData data, in Ray ray, half4 sdfRGBA, in Texture2DArray texPack, in SamplerState texPackSampler, out half3 normal)
{
	sdfRGBA = CalculateLitModelWrapper(data.litData, ray, sdfRGBA, texPack, texPackSampler, normal);
	return CalculateSceneRefraction(sdfRGBA, ray, normal, data.refractionIntensity, data.refractionOpacity, data.refractionInverse, data.refractionDensity);
}

// --------------------- Registered Core Method Contents
half4 CalculatePBRModel(in RMStandardPBRData data, in Ray ray, half4 sdfRGBA)
{
	half3 normals;
	return CalculatePBRModelWrapper(data, ray, sdfRGBA, RMStandardPBRDataTextures, sampler_RMStandardPBRDataTextures, normals);
}
...
// --------------------- Registered Post Method Contents
half4 CalculateReflections(in RMStandardPBRData data, in Ray ray, half4 sdfRGBA)
{
...
{
    sdfRGBA = CalculatePBRModel(RMStandardPBRDataInstance[0], ray, sdfRGBA);

    return sdfRGBA;
}
// -----------------------------------------------------------------------

// --------------------- Registered Post Method Global Contents
half4 CalculateReflections_Global(in RMStandardPBRData data, in Ray ray, half4 sdfRGBA)
{
#ifndef STANDARD_REFLECTION
	return sdfRGBA;
#endif
	...
	{ 
		...
		sdfMaterialInstance = sdf.w;
#endif
		//sdfRGBA += float4(RM_GlobalRenderMaterials(r, color, sdfMaterialType, sdfMaterialInstance).rgb, 0) * data.reflectionIntensity;
		sdfRGBA += float4(RM_PerObjRenderMaterials(r, color, sdfMaterialType, sdfMaterialInstance).rgb, 0) * data.reflectionIntensity;
	}
...
{
    sdfRGBA = CalculateReflections_Global(RMStandardPBRDataInstance[0], ray, sdfRGBA);
