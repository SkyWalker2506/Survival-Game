...
StructuredBuffer<RMStandardLitData> RMStandardLitDataInstance;
StructuredBuffer<RMStandardUnlitData> RMStandardUnlitDataInstance;

// --------------------- Registered Texture Containers
Texture2DArray RMStandardLitDataTextures;
SamplerState sampler_RMStandardLitDataTextures;
Texture2DArray RMStandardUnlitDataTextures;
SamplerState sampler_RMStandardUnlitDataTextures;
...
half4 CalculateLitModel(in RMStandardLitData data, in Ray ray, half4 sdfRGBA)
{
	half3 normals;
	return CalculateLitModelWrapper(data, ray, sdfRGBA, RMStandardLitDataTextures, sampler_RMStandardLitDataTextures, normals);
}
half4 CalculateUnlitModel(in RMStandardUnlitData data, in Ray ray, half4 sdfRGBA)
{
	half3 normals;
	return CalculateUnlitModelWrapper(data, ray, sdfRGBA, RMStandardUnlitDataTextures, sampler_RMStandardUnlitDataTextures, normals);
}
...
half4 RM_PerObjRenderMaterials(in Ray ray, half4 sdfRGBA, in int sdfMaterialType, in int sdfMaterialInstance)
{
    ...
    sdfRGBA = sdfMaterialType != 0 ? sdfRGBA : CalculateLitModel(RMStandardLitDataInstance[sdfMaterialInstance], ray, sdfRef);
...
{
    sdfRGBA = CalculateUnlitModel(RMStandardUnlitDataInstance[0], ray, sdfRGBA);
