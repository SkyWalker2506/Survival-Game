...
namespace Gaia.Pipeline.HDRP
{
    public static class GaiaHDRPRuntimeUtils
    {
...
#if HDPipeline
        /// <summary>
        /// Processes a default sun light state
        /// </summary>
        public static void SetupDefaultHDRPSunLight(Light light = null)
        {
            if (light == null)
            {
                light = GaiaUtils.GetMainDirectionalLight();
            }

            if (light != null)
            {
                HDAdditionalLightData data = GetHDLightData(light);
                if (data != null)
                {
                    data.EnableShadows(true);
                    data.RequestShadowMapRendering();
                    data.SetIntensity(70000f, LightUnit.Lux);
                    data.EnableColorTemperature(true);
                    data.SetColor(Color.white, 6570f);
                    data.SetShadowUpdateMode(ShadowUpdateMode.EveryFrame);
        ...
        {
            TerrainHelper.SetTerrainMaterial(profile.m_highDefinitionTerrainMaterial);
        }
        ...
        private static void SyncHDRPAmbientLight(VolumeProfile sceneProfile, VolumeProfile sourceProfile)
        {
            ...
            if (sceneProfile.TryGet(out IndirectLightingController visualEnvironmentScene))
            {
                if (sourceProfile.TryGet(out IndirectLightingController visualEnvironmentSource))
                {
                    ...
                    visualEnvironmentSource.indirectDiffuseLightingMultiplier.value = visualEnvironmentScene.indirectDiffuseLightingMultiplier.value;
                    visualEnvironmentSource.reflectionProbeIntensityMultiplier.value = visualEnvironmentScene.reflectionProbeIntensityMultiplier.value;
#else
