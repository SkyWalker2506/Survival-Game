...
namespace Gaia
{
    public static class GaiaWater
    {
        ...
        public static Material m_masterMaterial;
        ...
        public static void GetProfile(int selectedProfile, SceneProfile sceneProfile, bool spawnWater, bool updateSettingsOnly)
        {
            try
            {
                if (m_masterMaterial == null)
                {
                    m_masterMaterial = GaiaWater.GetGaiaOceanMaterial();
                }

                if (m_masterMaterial == null)
                {
                    Debug.LogError("Master material is null. To process the water update and changes the master material is required. Make sure you have the Gaia Ocean.mat material in your project.");
                    return;
                }
                ...
                else
                {
                    ...
                    if (sceneProfile.m_waterSystemMode == GaiaConstants.GlobalSystemMode.Gaia)
                    {
                        if (sceneProfile.m_selectedWaterProfileValuesIndex != -99 && sceneProfile.m_selectedWaterProfileValuesIndex < sceneProfile.m_waterProfiles.Count)
                        {
                            GaiaWaterProfileValues currentProfileValues = sceneProfile.m_waterProfiles[selectedProfile];
                            if (currentProfileValues == null)
                            {
                                return;
                            }

                            UpdateGlobalWater(m_masterMaterial, currentProfileValues, spawnWater, updateSettingsOnly, sceneProfile);
                        }
                        else
                        {
                            SetWaterToNoneState(sceneProfile, true);
                        }
                    }
                    else
                    {
                        SetWaterToNoneState(sceneProfile);
                    }
                }
            }
            catch (Exception e)
        ...
        public static void GetProfile(int selectedProfile, Material masterMaterial, GaiaWaterProfile waterProfile, bool spawnWater, bool updateSettingsOnly)
        {
            try
            {
                ...
                {
                    m_parentObject = GetOrCreateParentObject(GaiaConstants.gaiaWaterObject, true);
                    if (selectedProfile == waterProfile.m_waterProfiles.Count)
                    {
                    ...
                    {
                        if (selectedProfile <= waterProfile.m_waterProfiles.Count - 1)
                        {
                            GaiaWaterProfileValues currentProfileValues = waterProfile.m_waterProfiles[selectedProfile];
                            if (currentProfileValues == null)
                            {
                                return;
                            }

                            UpdateGlobalWater(masterMaterial, currentProfileValues, spawnWater, updateSettingsOnly, sceneProfile);
                        }
                    }
                }
            }
            ...
            try
            {
                if (profile.m_enableWaterMeshQuality)
                {
                    ...
                    else
                    {
                        ...
                        regenerate = WaterMeshNeedsRegenerated(waterGeneration, profile);
                        waterGeneration.m_MeshType = profile.m_meshType;
                        waterGeneration.m_customWaterMesh = profile.m_customWaterMesh;
        ...
        /// <summary>
        /// Checks to see if the water mesh size or density values have changed
        /// </summary>
        /// <param name="waterSystem"></param>
        /// <param name="profile"></param>
        /// <returns></returns>
        private static bool WaterMeshNeedsRegenerated(PWS_WaterSystem waterSystem, SceneProfile profile)
        {
            if ( waterSystem.m_MeshType != profile.m_meshType || waterSystem.m_customWaterMesh != profile.m_customWaterMesh || waterSystem.m_Size.x != profile.m_xSize || waterSystem.m_Size.z != profile.m_zSize || waterSystem.m_meshDensity.x != profile.m_customMeshQuality || waterSystem.m_meshDensity.y != profile.m_customMeshQuality)
            {
                return true;
            }
        ...
        /// <summary>
        ///  Sets the water to none state and removes systems
        /// </summary>
        /// <param name="sceneProfile"></param>
        /// <param name="setState"></param>
        private static void SetWaterToNoneState(SceneProfile sceneProfile, bool setState = false)
        {
            GaiaUtils.RemoveWaterSystems();
            if (setState)
            {
                sceneProfile.m_waterSystemMode = GaiaConstants.GlobalSystemMode.None;
                sceneProfile.m_selectedWaterProfileValuesIndex = 0;
            }
        }
        /// <summary>
        ...
        {
#if HDPipeline
            GameObject runtimeGO = GaiaUtils.GetRuntimeSceneObject(false);
            if (runtimeGO != null)
            {
                foreach (Transform t in runtimeGO.transform)
                {
                    if (t.name.StartsWith("Unity HDRP Water"))
                    {
                        return true;
                    }
                }
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (waterObject == null)
                    {
                        ...
                        waterObject.transform.position = new Vector3(waterObject.transform.position.x, seaLevel, waterObject.transform.position.z);
                    }
                    else
                    {
        ...
        public static void UpdateWaterSeaLevel(GameObject waterPrefab, float seaLevel)
        {
            try
            {
                ...
                GaiaUnderwaterEffects underwaterEffects = GameObject.FindAnyObjectByType<GaiaUnderwaterEffects>();
                if (underwaterEffects != null)
                {
                    underwaterEffects.m_seaLevel = seaLevel;
                }
        ...
        public static void SetupWaterReflectionSettings(SceneProfile profile, GaiaWaterProfileValues waterProfileValues, bool forceUpdate)
        {
            try
            {
                ...
                PWS_WaterSystem reflection = Object.FindAnyObjectByType<PWS_WaterSystem>();
                if (reflection != null)
                {
                    ...
                    if (GaiaUtils.GetActivePipeline() != GaiaConstants.EnvironmentRenderer.HighDefinition)
                    {
                        GaiaPlanarReflections planarReflections = Object.FindAnyObjectByType<GaiaPlanarReflections>();
                        if (planarReflections != null)
                        {
                            ...
                            EditorUtility.SetDirty(planarReflections);
                        }
                    }
                    else
                    {
                        ...
                        if (hdrpPlanarReflection != null)
                        {
                            ...
                            hdrpPlanarReflection.settingsRaw.roughReflections = profile.m_roughReflectionsHDRP;
                            hdrpPlanarReflection.gameObject.transform.position = new Vector3(0f, reflection.SeaLevel + profile.m_clipPlaneOffset, 0f);
                            // hdrpPlanarReflection.settings.cameraSettings.renderingPathCustomFrameSettings.SetEnabled(FrameSettingsField.ShadowMaps, profile.m_reflectShadowsHDRP);
                            // hdrpPlanarReflection.frameSettings.SetEnabled(FrameSettingsField.ShadowMaps, profile.m_reflectShadowsHDRP);
                        }
#endif
                    }
                }
        ...
        private static void UpdateWaterMaterialInstances(Material masterMaterial, Material underwaterMaterial, bool singleMaterial = false)
        {
            try
            {
                ...
                if (!singleMaterial)
                {
                    ...
                    {
                        PWS_WaterSystem waterSystem = GameObject.FindAnyObjectByType<PWS_WaterSystem>();
                        if (waterSystem != null)
                        {
